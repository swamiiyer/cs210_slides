...
\begin{frame}[fragile]
A \emph{linked list} is a recursive data structure that is either empty (\lstinline{null}) or a reference to a node having a generic item and a reference to a linked list
...
The \lstinline{Bag}, \lstinline{Queue}, and \lstinline{Stack} APIs can also be implemented using resizing arrays, in which case, the amortized cost for any sequence of operations is constant in the worst case
\end{frame}
\end{document}
